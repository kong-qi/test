layui.define(['uupload', 'layer', 'layerOpen'], function (exports) {
    ...
    function uploadPlaceUp(obj, accept_type) {
        ...
        return uploadApi(that, file_type, accept_type, function (res) {
            //找到父
            var html = '<div class="col-auto mb-2 upload-area-more-item"' +
                'data-tmp_name="' + res.tmp_name + '"' +
                'data-ext="' + res.ext + '" data-type="' + res.type + '"' +
                'data-path="' + res.path + '" ' +
                ...
                '<div class="card"> ' +
                '<div class="card-bdoy">' +
                '<div class="h6rem"><img class="img-fluid  maxH6rem" src="' + res.view_src + '" src="" alt=""></div>\n' +
                '<div class="card-title text-center mt-2" title="' + res.tmp_name + '">' + res.limit_tmp_name + '</div>\n' +
                '<span class="badge badge-primary "><i class="fa fa-check-circle"></i></span>' +
                '</div></div>\n' +
                '</div>';

            var parentObj = $(that.data("target"));
            parentObj.find('.row').prepend(html);
    ...
    function uploadMore(obj, accept_type) {
        ...
        return uploadApi(that, file_type, accept_type, function (res) {
            ...
            var html = '<div class="col-auto mb-2 upload-area-more-item"' +
                ...
                'data-oss_type="' + res.oss_type + '" ' +
                'data-origin_path="' + res.origin_path + '">\n' +
                '    <div class="h6rem"><img class="img-fluid img-thumbnail maxH6rem" src="' + res.view_src + '" src="" alt=""></div>\n' +
                '    <div class="handle d-flex justify-content-center">\n' +
                '        <button type="button" class="btn btn-sm js_left_pic" data-tips="tooltip" ' +
                'title="' + appLang.trans('左移') + '">\n' +
                '            <i class="fa fa-chevron-left"></i> </button>\n' +
                '        <button type="button" class="btn btn-sm js_right_pic" data-tips="tooltip" title="' + appLang.trans('右移') + '">\n' +
                '            <i class="fa fa-chevron-right"></i> </button>\n' +
                '<button type="button" class="btn btn-sm js_remove_pic" data-tips="tooltip" title="' + appLang.trans('移除') + '" >\n' +
                ' <i class="fa fa-trash"></i> </button>\n' +
                '</div>\n' +
                '</div>';

            var parentObj = $(that.data("target"));
            parentObj.removeClass('d-none').find('.row').append(html);
    ...
    function iconPlace(that, is_more) {
        ...
        uploadPlaceApi(g_icon_url, function (layero, index) {
            var items = layero.find('iframe').contents().find('.fa-hover.active');
            ...
            //找到父
            var parentObj = $(that.data("target"));
            parentObj.find(".iupload-area-img-show").removeClass('d-none').empty();
            //如果是文件，则需要输出文件名
