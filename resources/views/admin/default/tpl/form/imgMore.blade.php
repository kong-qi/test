<div class="upload-area">
    ...
    <div class="mb-10 upload-area-more {{ (!isset($form_item['data']) && empty($form_item['data']))?'none':'' }}"
         id="{{ (md5($form_item['field'])) }}">
        ...
        <div class="row">
            ...
            @if(!empty($form_item['data']))
                @foreach ($form_item['data'] as $k => $v)
                    ...
                    @endphp
                    <div class="col-auto mb-2 upload-area-more-item"
                        ...
                        <div class="h6rem">
                            <img class="img-fluid img-thumbnail maxH6rem" src="{{$v['view_src'] ?? ''}}" alt="">
                        </div>
                        <div class="handle d-flex justify-content-center">
                            ...
                            <button type="button" class="btn btn-sm js_right_pic" data-tips="tooltip"
                                    title="{{ lang('右移') }}">
                                <i class="fas fa fa-chevron-right"></i></button>
                            <button type="button" class="btn btn-sm js_remove_pic" data-tips="tooltip"
                                    title="{{ lang('移除') }}">
    ...
    <div class="d-flex">
        ...
        id="upload{{ (md5($form_item['field'])) }}"
                ...
                class="btn btn-primary btn-sm  mr-2"><i class="fas fa-upload fa-fw"></i> {{ lang('点击上传') }}</button>
        <button type="button" data-more="1" data-event="uploadPlace"
                {{ $form_item['place_attr']??'' }}
                data-target="#{{ (md5($form_item['field']))  }}" class="btn btn-primary btn-sm"><i
                    class="fas fa fa-cloud"></i> {{ lang('库选择') }}</button>
